...
// These tests require the 'snd-dummy' kernel module: sudo modprobe snd-dummy
...
func testControlValues(t *testing.T, m *alsa.Mixer) {
	...
	for _, ctl := range m.Ctls {
		...
		// INT64 accessors must error on non-INT64 controls
...
func testInt64Ctl(t *testing.T, ctl *alsa.MixerCtl) {
	...
	// Some controls accept writes but their value does not actually change.
...
func testEnumCtl(t *testing.T, ctl *alsa.MixerCtl) {
	// A control can be misidentified as ENUM with no items; nothing to test then.
	numEnums, err := ctl.NumEnums()
	if err != nil || numEnums == 0 {
		return
	}
	...
	// Need at least two items to set a value different from the original.
	if numEnums < 2 {
		return
	}
	...
	for i := uint(0); i < uint(ctl.NumValues()); i++ {
		...
		assert.NoError(t, err)
		// Some drivers ignore the write, so accept the original value too.
...
func testMixerEvents(t *testing.T, m *alsa.Mixer) {
	...
	// Prefer a writable integer "Volume" control to trigger an event.
	...
	// Toggle the value between min and max in a goroutine to trigger an event.
	go func() {
		defer wg.Done()
	...
	// Trigger another event by restoring the value, then consume and discard it.
	err = targetCtl.SetValue(0, originalVal)
	if err != nil {
		return
	}
...
func testMixerCtlByNameAndDevice(t *testing.T, m *alsa.Mixer) {
	...
	// Valid control name but a device number it does not exist on.
	if len(m.Ctls) > 0 {
		firstCtl := m.Ctls[0]
		name := firstCtl.Name()

		nonExistentDevice := uint32(9999)
		isUnique := false
